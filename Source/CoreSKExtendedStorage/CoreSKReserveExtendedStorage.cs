...
namespace Share_The_Load
{
	...
	public static class ExtendedStoragePatches_SK
	{
		static ExtendedStoragePatches_SK()
		{
			if (!ModCompatibilityCheck.ExtendedStorageSKIsActive) return;
			Log.Message($"Share The Load patching with Extended Storage SK!");

			HarmonyInstance harmony = HarmonyInstance.Create("Uuugggg.rimworld.Share_The_Load-ES-SK.main");
			harmony.Patch(AccessTools.Method(typeof(ReservationManager), "CanReserve"),
				new HarmonyMethod(typeof(CanReserve_Patch_ES_SK), "Prefix"), null);
			harmony.Patch(AccessTools.Method(typeof(ReservationManager), "Reserve"),
				new HarmonyMethod(typeof(Reserve_Patch_ES_SK), "Prefix"), null);
			harmony.Patch(AccessTools.Method(typeof(ReservationManager), "Release"),
				new HarmonyMethod(typeof(Release_Patch_ES_SK), "Prefix"), null);
			harmony.Patch(AccessTools.Method(typeof(ReservationManager), "ReleaseClaimedBy"),
				new HarmonyMethod(typeof(ReleaseClaimedBy_Patch_ES_SK), "Prefix"), null);
		}
	}

	//[HarmonyPatch(typeof(ReservationManager), "CanReserve")]
	static class CanReserve_Patch_ES_SK
	{
		...
		public static bool Prefix(Pawn claimant, LocalTargetInfo target, ref bool __result)
		{
			if (claimant.IsFreeColonist && target.Cell != LocalTargetInfo.Invalid)
			{
				if(claimant.Map.thingGrid.ThingsAt(target.Cell).FirstOrDefault(t => t is Building_ExtendedStorage) is Thing thing)
				{
					...
					int canDo = storage.ApparentMaxStorage - storage.StoredThing?.stackCount ?? 0;
	...
	static class Reserve_Patch_ES_SK
	{
	...
	static class Release_Patch_ES_SK
	{
	...
	static class ReleaseClaimedBy_Patch_ES_SK
	{
